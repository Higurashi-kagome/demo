...
/**
 * Com21EventListener类使用“事件监听模式”监听串口COM1，
 * 并通过COM1的输入流对象来获取该端口接收到的数据（在本文中数据来自串口COM11）。
 * 使用“事件监听模式”监听串口，必须字定义一个事件监听类，该类实现SerialPortEventListener
 * 接口并重写serialEvent方法，在serialEvent方法中编写监听逻辑。
 */
public class TestEventListener implements SerialPortEventListener {
    ...
    public TestEventListener(){
        try {
            ...
            serialCom1 = (SerialPort) com1.open("TestEventListener", 1000);
            inputStream = serialCom1.getInputStream();
            //向串口添加事件监听对象。
            serialCom1.addEventListener(this);
    ...
    public void serialEvent(SerialPortEvent event) {
        ...
        //记录已经到达串口COM1且未被读取的数据的字节（Byte）数。
